...
local function getSocietyAccounts()
    local societies = {}

    local job = QBX.PlayerData.job
    if job and job.bankAuth then
    ...
    if gang and gang.name ~= 'none' and gang.bankAuth then
        societies[#societies + 1] = {
            id = gang.name,
...
RegisterNUICallback('buyVehicle', function(data, cb)
    TriggerServerEvent('qb-occasions:server:buyVehicle', currentVehicle, data and data.account)
    cb('ok')
end)
