...
RegisterNetEvent('qb-occasions:server:buyVehicle', function(vehicleData, paymentAccount)
    local src = source
    local player = exports.qbx_core:GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',{vehicleData.plate, vehicleData.oid})
    if not result[1] or not next(result[1]) then return end
    local price = result[1].price
    local useSociety = type(paymentAccount) == 'string' and paymentAccount ~= ''
    local paymentLabel
    local buyerCharacterName = ('%s %s'):format(player.PlayerData.charinfo.firstname, player.PlayerData.charinfo.lastname)

    if useSociety then
        if GetResourceState('Renewed-Banking') ~= 'started' then
            exports.qbx_core:Notify(src, locale('error.society_account_unavailable'), 'error', 3500)
            return
        end

        local job = player.PlayerData.job
        local gang = player.PlayerData.gang
        if job and job.name == paymentAccount and job.bankAuth then
            paymentLabel = job.label
        elseif gang and gang.name == paymentAccount and gang.bankAuth then
            paymentLabel = gang.label
        else
            exports.qbx_core:Notify(src, locale('error.society_no_access'), 'error', 3500)
            return
        end

        local accountBalance = exports['Renewed-Banking']:getAccountMoney(paymentAccount)
        if not accountBalance or accountBalance < price then
            exports.qbx_core:Notify(src, locale('error.society_not_enough_money'), 'error', 3500)
            return
        end

        local removed = exports['Renewed-Banking']:removeAccountMoney(paymentAccount, price)
        if not removed then
            exports.qbx_core:Notify(src, locale('error.society_not_enough_money'), 'error', 3500)
            return
        end
        ...
        exports['Renewed-Banking']:handleTransaction(
            paymentAccount,
            locale('info.used_vehicle_lot'),
            price,
            locale('info.society_vehicle_purchase'):format(result[1].plate),
            paymentLabel or paymentAccount,
            buyerCharacterName,
            'withdraw'
        )
    else
    ...
    local newPrice = math.ceil((price / 100) * 77)
    MySQL.insert(
    ...
    local buyerName = buyerCharacterName
    local paymentSource = useSociety and (paymentLabel or paymentAccount) or locale('info.personal_account')
    TriggerEvent('qb-log:server:CreateLog', 'vehicleshop', 'bought', 'green', ('**%s** purchased for %s (%s) from **%s** using %s funds'):format(buyerName, price, result[1].plate, sellerCitizenId, paymentSource))
