...
local function hasLighter(source)
    return exports.ox_inventory:Search(source, 'count', 'lighter') > 0
end

local function ensureLighter(source)
    if hasLighter(source) then return true end

    exports.qbx_core:Notify(source, locale('error.need_lighter'), 'error')
    return false
end
...
exports.qbx_core:CreateUseableItem('joint', function(source)
    if not ensureLighter(source) then return end
