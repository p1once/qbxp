...
lib.callback.register('qbx_management:server:updateSalary', function(source, citizenId, grade, salary)
    local player = exports.qbx_core:GetPlayer(source)
    ...
    if grade > gradeLevel then
        exports.qbx_core:Notify(source, locale('error.cant_promote'), 'error')
        return
    end

    if type(salary) ~= 'number' then
        exports.qbx_core:Notify(source, locale('error.invalid_salary'), 'error')
        return
    end
    ...
    if not (job and job.grades[grade]) then
        exports.qbx_core:Notify(source, locale('error.grade_not_exist'), 'error')
        return
    end

    job.grades[grade].payment = salary
    exports.qbx_core:UpsertJobGrade(jobName, grade, job.grades[grade])

    exports.qbx_core:Notify(source, locale('success.salary_updated', job.grades[grade].name, salary), 'success')

    local employee = exports.qbx_core:GetPlayerByCitizenId(citizenId)
    if employee then
        exports.qbx_core:Notify(employee.PlayerData.source, locale('success.new_salary', salary), 'success')
    end

    logger.log({
        source = source,
        event = 'qbx_management:server:updateSalary',
        message = locale('logs.updated_salary', citizenId, grade, jobName, salary),
        webhook = config.discordWebhook
    })
end)
