...
local function manageEmployee(player, groupName, groupType)
    ...
    if groupType == 'job' then
        local currentGrade = employeeLoop[player.grade]
        if currentGrade then
            table.insert(employeeMenu, {
                title = locale('menu.adjust_salary'),
                description = locale('menu.current_salary', currentGrade.payment),
                icon = 'money-check-dollar',
                onSelect = function()
                    local dialog = lib.inputDialog(locale('menu.adjust_salary_title', currentGrade.name), {
                        { type = 'number', label = locale('grade.pay'), default = currentGrade.payment, required = true, min = 0 },
                    })

                    if not dialog then return end

                    local wage = tonumber(dialog[1])
                    if not wage then return end

                    lib.callback.await('qbx_management:server:updateSalary', false, player.cid, player.grade, wage)
...
local function employeeList(groupType)
    ...
    for _, employee in pairs(employees) do
        ...
        if groupType == 'job' then
            employeesData.metadata = employeesData.metadata or {}
            employeesData.metadata[#employeesData.metadata + 1] = {
                label = locale('menu.salary'),
                value = ('$%s'):format(JOBS[groupName].grades[employee.grade].payment)
            }
        end
