...
const app = Vue.createApp({
  data: function() {
      ...
      isDynamicStrengthChecked: this.initIsDynamicStrengthChecked(),
      isDynamicEnduranceChecked: this.initIsDynamicEnduranceChecked(),
  ...
  watch: {
    ...
    isDynamicStrengthChecked: function() {
                        localStorage.setItem("isDynamicStrengthChecked", this.isDynamicStrengthChecked);
                },
    isDynamicEnduranceChecked: function() {
                        localStorage.setItem("isDynamicEnduranceChecked", this.isDynamicEnduranceChecked);
                },
  ...
  methods: {
    ...
    initIsDynamicStrengthChecked: function() {
                        const stored = localStorage.getItem("isDynamicStrengthChecked");
                        if (stored === null) {
                                return false;
                        } else {
                                return stored == 'true';
    ...
    initIsDynamicEnduranceChecked: function() {
                        const stored = localStorage.getItem("isDynamicEnduranceChecked");
                        if (stored === null) {
                                return false;
                        } else {
                                return stored == 'true';
    ...
    dynamicStrength: function(event) {
      targetId = event.currentTarget.id;
      dynamicStrength()
    },
    dynamicEndurance: function(event) {
      targetId = event.currentTarget.id;
      dynamicEndurance()
    },
...
function dynamicStrength() {
  $.post(`https://${GetParentResourceName()}/dynamicStrength`);
}
function dynamicEndurance() {
  $.post(`https://${GetParentResourceName()}/dynamicEndurance`);
}
...
const playerHud = {
  ...
  methods: {
    hudTick(data) {
      ...
      this.updateOxygenColor(data.oxygen);
      ...
      this.dynamicEndurance = data.dynamicEndurance;
      ...
      if (data.dynamicStrength == true) {
        if (data.strength >= 100) {
          this.showStrength = false;
        } else {
          this.showStrength = true;
        }
      } else if (data.dynamicStrength == false) {
        this.showStrength = true;
      }

      if (data.dynamicEndurance == true) {
        if (data.endurance >= 100) {
          this.showEndurance = false;
        } else {
          this.showEndurance = true;
        }
      } else if (data.dynamicEndurance == false) {
        this.showEndurance = true;
      }

      if (data.strength >= 80) {
        this.strengthColor = "#3FA554";
      } else if (data.strength <= 30) {
        this.strengthColor = "#ff0000";
      } else {
        this.strengthColor = "#c97b1a";
      }

      if (data.endurance >= 80) {
        this.enduranceColor = "#1fb27d";
      } else if (data.endurance <= 30) {
        this.enduranceColor = "#ff0000";
      } else {
        this.enduranceColor = "#1a7cad";
      }
    ...
    updateOxygenColor(value) {
      const clamped = Math.max(0, Math.min(100, value ?? 0));
      const ratio = clamped / 100;
      const start = { r: 138, g: 168, b: 189 };
      const end = { r: 255, g: 64, b: 64 };
