...
local strength = playerState.strength or 50
local endurance = playerState.endurance or 50
local cashAmount = 0
local bankAmount = 0
local nitroActive = 0
local hp = 100
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    strength = QBX.PlayerData.metadata.strength
    endurance = QBX.PlayerData.metadata.endurance
...
RegisterNUICallback('dynamicStrength', function(_, cb)
    Wait(50)
    sharedConfig.menu.isDynamicStrengthChecked = not sharedConfig.menu.isDynamicStrengthChecked
    saveSettings()
    cb('ok')
end)

RegisterNUICallback('dynamicEndurance', function(_, cb)
    Wait(50)
    sharedConfig.menu.isDynamicEnduranceChecked = not sharedConfig.menu.isDynamicEnduranceChecked
    saveSettings()
    cb('ok')
end)
...
AddStateBagChangeHandler('strength', ('player:%s'):format(cache.serverId), function(_, _, value)
    strength = value
end)

AddStateBagChangeHandler('endurance', ('player:%s'):format(cache.serverId), function(_, _, value)
    endurance = value
end)
...
local function updatePlayerHud(data)
    ...
    for key, value in pairs(data) do
        if prevPlayerStats[key] ~= value then
            shouldUpdate = true
            break
        end
    end
    ...
    if shouldUpdate then
        prevPlayerStats = shallowCopy(data)
        local payload = shallowCopy(data)
        payload.action = 'hudtick'
...
CreateThread(function()
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            -- Stamina
            local fitnessState = LocalPlayer.state
            if not IsEntityInWater(cache.ped) then
                if fitnessState and fitnessState.fitnessOxygenActive then
                    oxygen = math.max(math.min(fitnessState.fitnessOxygen or oxygen, 100), 0)
                else
            ...
            if not (cache.vehicle and not IsThisModelABicycle(cache.vehicle)) then
                local speed = cache.vehicle and math.ceil(GetEntitySpeed(cache.vehicle) * speedMultiplier) or 0
                updatePlayerHud({
                    show = show,
                    dynamicHealth = sharedConfig.menu.isDynamicHealthChecked,
                    dynamicArmor = sharedConfig.menu.isDynamicArmorChecked,
                    dynamicHunger = sharedConfig.menu.isDynamicHungerChecked,
                    dynamicThirst = sharedConfig.menu.isDynamicThirstChecked,
                    dynamicStress = sharedConfig.menu.isDynamicStressChecked,
                    dynamicStrength = sharedConfig.menu.isDynamicStrengthChecked,
                    dynamicEndurance = sharedConfig.menu.isDynamicEnduranceChecked,
                    dynamicOxygen = sharedConfig.menu.isDynamicOxygenChecked,
                    dynamicEngine = sharedConfig.menu.isDynamicEngineChecked,
                    dynamicNitro = sharedConfig.menu.isDynamicNitroChecked,
                    ...
                    harness = LocalPlayer.state?.harness,
                    hp = hp,
                    speed = speed,
                    engine = -1,
                    cinematic = sharedConfig.menu.isCineamticModeChecked,
                    dev = dev,
                })
            end
            -- Vehicle hud
