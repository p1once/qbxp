...
local function sanitizeKeybindInput(value)
    if type(value) ~= 'string' then return nil end

    local sanitized = value:gsub('%s+', ''):upper()
    if sanitized == '' or not sanitized:match('^[A-Z0-9_]+$') then return nil end

    return sanitized
end

local function buildAdminOptionsMenuOptions()
    return {
        {
            label = locale('options_menu.label1', GetAdminMenuKeybind()),
            description = locale('options_menu.desc1'),
            icon = 'fas fa-keyboard',
            close = false,
        },
        {
            label = locale('options_menu.label2', GetAdminMenuDefaultKey()),
            description = locale('options_menu.desc2'),
...
local function updateAdminOptionsMenuLabels()
    local options = buildAdminOptionsMenuOptions()
    lib.setMenuOptions('qbx_adminmenu_options_menu', options)

    local selectedIndex = MenuIndexes.qbx_adminmenu_options_menu or 1
    if selectedIndex < 1 or selectedIndex > #options then
        selectedIndex = 1
    end

    MenuIndexes.qbx_adminmenu_options_menu = selectedIndex
    return selectedIndex
end

local function openAdminOptionsMenu()
    local selectedIndex = updateAdminOptionsMenuLabels()
    lib.showMenu('qbx_adminmenu_options_menu', selectedIndex)
end

local function startAdminKeyCapture()
    lib.hideMenu(false)
    SetNuiFocus(true, true)
    SendNUIMessage({ action = 'openKeyCapture', hint = nil })
end

RegisterNUICallback('keyCaptured', function(data, cb)
    cb({})
    SendNUIMessage({ action = 'closeKeyCapture' })
    SetNuiFocus(false, false)

    if data and data.cancel then
        return openAdminOptionsMenu()
    end

    local key = data and data.key or nil
    if not key or type(key) ~= 'string' then
        exports.qbx_core:Notify(locale('options_menu.invalid_key'), 'error')
        return openAdminOptionsMenu()
    end

    local chosenKey = sanitizeKeybindInput(key)
    if not chosenKey then
        exports.qbx_core:Notify(locale('options_menu.invalid_key'), 'error')
    else
        SetAdminMenuKeybind(chosenKey)
        exports.qbx_core:Notify(locale('options_menu.saved', chosenKey), 'success')
    end
...
lib.registerMenu({
    id = 'qbx_adminmenu_options_menu',
    title = locale('title.options_menu'),
    position = 'top-right',
    useMouse = true,
    onClose = function(keyPressed)
        CloseMenu(false, keyPressed, 'qbx_adminmenu_admin_menu')
    end,
    onSelected = function(selected)
        MenuIndexes.qbx_adminmenu_options_menu = selected
    end,
    options = buildAdminOptionsMenuOptions()
}, function(selected)
    if selected == 1 then
        startAdminKeyCapture()
        return
    elseif selected == 2 then
        -- Hide to guarantee the selection triggers reliably
        lib.hideMenu(false)
        ResetAdminMenuKeybind()
        exports.qbx_core:Notify(locale('options_menu.reset', GetAdminMenuKeybind()), 'success')
        return openAdminOptionsMenu()
    end
end)
...
local options = {
    ...
    function()
        -- Ensure we fully switch focus to the Options submenu
        lib.hideMenu(false)
...
lib.registerMenu({
    ...
    options = {
        ...
        {label = locale('admin_options.label12'), description = locale('admin_options.desc12'), icon = 'fas fa-sliders', close = false},
    }
