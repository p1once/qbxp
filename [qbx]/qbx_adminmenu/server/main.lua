...
local playerDataOptions = {
    ...
    job = function(target, input)
        local jobName = input[1]
        local grade = tonumber(input[2]) or 0
...
lib.callback.register('qbx_admin:server:getJobOptions', function(source)
    if not IsPlayerAceAllowed(source, config.eventPerms.changePlayerData) then return end
    if not exports.qbx_core:IsOptin(source) then return end

    local jobs = exports.qbx_core:GetJobs()
    if not jobs then return end

    local jobOptions, gradeOptions = {}, {}

    for jobName, jobData in pairs(jobs) do
        jobOptions[#jobOptions + 1] = {
            value = jobName,
            label = jobData.label and string.format('%s (%s)', jobData.label, jobName) or jobName
        }

        local grades = {}
        if jobData.grades then
            for gradeLevel, gradeData in pairs(jobData.grades) do
                local numericGrade = tonumber(gradeLevel) or gradeLevel
                grades[#grades + 1] = {
                    value = tostring(numericGrade),
                    label = gradeData.name and string.format('%s - %s', numericGrade, gradeData.name) or tostring(numericGrade)
                }
            end
        ...
        gradeOptions[jobName] = grades
    end

    table.sort(jobOptions, function(a, b)
