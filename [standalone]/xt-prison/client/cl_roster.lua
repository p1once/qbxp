...
local function removeRosterZone()
    if rosterZone then
        local ok
        if exports and exports.ox_target and exports.ox_target.zoneExists then
            ok = exports.ox_target:zoneExists(rosterZone)
        end
        if not ok and type(rosterZone) == 'string' then
            ok = true -- allow removing by name even if zoneExists not available
        end
        if ok then
