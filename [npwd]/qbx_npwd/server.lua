...
-- Avoid double-unload when NPWD's built-in QBX bridge is active.
-- NPWD listens to `QBCore:Server:OnPlayerUnload` and will unload players itself
-- when `set npwd:framework "qbx"` (or qb) is configured. In that case, skip our manual unload.
RegisterNetEvent('qbx_npwd:server:UnloadPlayer', function()
    local framework = GetConvar('npwd:framework', ''):lower()
...
if Config.PhoneAsItem then
    for i = 1, #Config.PhoneList do
        exports.qbx_core:CreateUseableItem(Config.PhoneList[i], function(source)
